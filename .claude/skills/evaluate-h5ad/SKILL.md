...
- Provenance: pull `get_descriptive_stats.columns[<col>].unique` for each of `donor_id` / `sample_id` / `library_id` (the stats are nested under a `columns` dict keyed by column name) and render as `N donors · M samples · K libraries`. Omit any metric whose column is absent OR whose column is present but unpopulated (i.e. `columns[<col>].unique == 0`, equivalently `columns[<col>].n_nan == n_rows`) — `unique` drops NaNs, so an all-NaN column would otherwise render as "0 libraries".
