...
- **`unsupported_count > 0` — compatibility.** Report it as an issue and name it: these elements can be **read but not written back**, so a tool that rewrites one fails at the write — after a multi-gigabyte snapshot has already been copied — instead of failing here. Give the count, and the sample paths in `unsupported` — note that when `unsupported_truncated` is true the list is a sample and `unsupported_count` is the real total, so cite the count, never the length of the list. `nullable-string-array` is the encoding this normally means; it is legal AnnData that our tooling can read since hca-validation-tools#637 but cannot yet write (hca-validation-tools#641), **not** a defect in the file. The reported paths are on-disk HDF5 paths, so they can be pasted straight into h5py or grep.

  Note the scope: what a flagged file blocks is the rewriting tools — `rename_cell_ids`, `compress_h5ad`. Inspection, `convert_cellxgene_to_hca` and `backfill_obs_from_source` copy rather than rewrite in place and run fine. The check covers dataframe *indexes* and categorical `categories`; a plain nullable string obs *column* is not reported, so a clean result is not a guarantee that every tool will succeed — it rules out the failure mode that has actually bitten us.
