...
**Check `get_storage_info.encodings` before running any fix.** If `unsupported_count` is greater than 0, **stop and report it rather than starting the fix sequence**: the named elements use an on-disk encoding our tools can read but **cannot write back**, so any tool that would rewrite one refuses by name instead of running. Give the count and the sample paths (they are real HDF5 paths) and say that hca-validation-tools#641 tracks the write support. The tool classes (per `docs/anndata-tools-contract.md`) determine what happens: **read-only** and **copy-and-transplant** tools are unaffected since hca-validation-tools#637 and read any source; every **in-place surgical** tool (the snapshot-then-rewrite class — rename, merge, backfill targets, placeholder replacement, CAP copy, and whatever joins that class next) refuses up front, before its snapshot; every **full-rewrite** tool (whatever streams through `write_h5ad` — compress, normalize, ...) refuses at that shared funnel, before any bytes are written. No tool of any class leaves a partial output file.
