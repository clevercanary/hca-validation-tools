...
**`get_storage_info.encodings` is informational since hca-validation-tools#641 — nothing refuses these encodings.** If `unsupported_count` is greater than 0, the named elements use a nullable-string encoding, and **every write normalizes what it touches**: full-rewrite tools (whatever streams through `write_h5ad` — `compress_h5ad`, `normalize_raw`, `populate_labels`, ...) normalize every such element and report the locations under `encodings_normalized`; in-place tools (rename, merge, backfill) normalize the elements they rewrite as part of the same write; `convert_cellxgene_to_hca` normalizes its whole output. Just note the count and proceed with the fix sequence — the flags clear as writes happen. Two exceptions. **Masked (null) string values** are a hard stop: a tool that meets one refuses by name, because they have no plain representation — raise it with the wrangler rather than flattening it. An unopenable file no longer shows up here at all: since hca-validation-tools#661 every tool opens through anndata first, so `get_storage_info` on a file anndata cannot read — masked categories, a truncated download — returns `{"error": ...}` carrying anndata's own message, never an `encodings` map. Treat that error as the verdict: the file is out of scope for these tools and the fix belongs upstream with the producer (`docs/anndata-tools-contract.md`, Scope). We are not in the diagnosis business, so do not reach for another tool to find out why — none of them will answer either. No tool leaves a partial output — an edit either fully applies or leaves the file untouched. That is not the same as a usable one: a *complete* snapshot of an out-of-scope file is still unopenable, so a clean run there is not a clean verdict.
