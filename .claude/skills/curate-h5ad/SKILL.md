...
**`get_storage_info.encodings` is informational since hca-validation-tools#641 — nothing refuses these encodings.** If `unsupported_count` is greater than 0, the named elements use a nullable-string encoding, and **every write normalizes what it touches**: full-rewrite tools (whatever streams through `write_h5ad` — `compress_h5ad`, `normalize_raw`, `populate_labels`, ...) normalize every such element and report the locations under `encodings_normalized`; in-place tools (rename, merge, backfill) normalize the elements they rewrite as part of the same write; `convert_cellxgene_to_hca` normalizes its whole output. Just note the count and proceed with the fix sequence — the flags clear as writes happen. The one hard stop: **masked (null) string values** — every tool that would rewrite the element refuses by name, because they have no plain representation; that is a data problem to raise with the wrangler, not an encoding to flatten. One documented exception: masked *categories* (a `.../categories` path) are read NA-tolerantly by `rename_cell_ids`, `backfill_obs_from_source`' source side, and `validate_marker_genes`, which proceed rather than refusing — a known, accepted gap (`docs/anndata-tools-contract.md`), so on such a file a clean run is not a clean verdict. No tool of any class leaves a partial output file.
