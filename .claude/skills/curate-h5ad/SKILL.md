...
**Check `get_storage_info.encodings` before running any fix.** If `unsupported_count` is greater than 0, the named elements use a nullable-string encoding that cannot be rewritten *in place* — but since hca-validation-tools#641 it is **repairable**: any **full-rewrite** tool (whatever streams through `write_h5ad` — `compress_h5ad`, `normalize_raw`) normalizes every such element to plain `string-array` on the way through and reports the locations under `encodings_normalized`. So instead of stopping, fold a full rewrite into the fix sequence early (compression is usually needed anyway) and the flag clears; **in-place surgical** tools (rename, merge, backfill targets, and whatever joins that class) refuse until then, naming that exact remedy; **read-only** and **copy-and-transplant** tools are unaffected and read any source (`convert_cellxgene_to_hca` normalizes its own output). The one hard stop that remains: **masked (null) string values** refuse by name everywhere — they have no plain representation, and that is a data problem to raise with the wrangler, not an encoding to flatten. No tool of any class leaves a partial output file.
