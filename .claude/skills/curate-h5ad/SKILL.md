...
**`get_storage_info.encodings` is informational since hca-validation-tools#641 — nothing refuses these encodings.** If `unsupported_count` is greater than 0, the named elements use a nullable-string encoding, and **every write normalizes what it touches**: full-rewrite tools (whatever streams through `write_h5ad` — `compress_h5ad`, `normalize_raw`, `populate_labels`, ...) normalize every such element and report the locations under `encodings_normalized`; in-place tools (rename, merge, backfill) normalize the elements they rewrite as part of the same write; `convert_cellxgene_to_hca` normalizes its whole output. If `masked` is empty, just note the count and proceed with the fix sequence — those flags clear as writes happen. If `masked` is non-empty, **stop and report it before any fix**: those paths hold masked (null) string values, which refuse by name in every write that touches them and never clear — they have no plain representation, and that is a data problem to raise with the wrangler (give each path and its count), not an encoding to flatten. No tool of any class leaves a partial output file.
