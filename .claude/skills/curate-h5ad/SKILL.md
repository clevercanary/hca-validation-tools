...
**`get_storage_info.encodings` is informational since hca-validation-tools#641 — nothing refuses these encodings.** If `unsupported_count` is greater than 0, the named elements use a nullable-string encoding, and **every write normalizes what it touches**: full-rewrite tools (whatever streams through `write_h5ad` — `compress_h5ad`, `normalize_raw`, `populate_labels`, ...) normalize every such element and report the locations under `encodings_normalized`; in-place tools (rename, merge, backfill) normalize the elements they rewrite as part of the same write; `convert_cellxgene_to_hca` normalizes its whole output. If `masked` is empty, just note the count and proceed with the fix sequence — those flags clear as writes happen (`masked` shares the report's scope: a masked element in `varm` or `uns` still refuses writes even though it is not reported, so a mid-sequence masked refusal is still possible and stops the run there). If `masked` is non-empty, **stop and report it before any fix that would rewrite those elements**: they hold masked (null) values with no plain representation. For a masked **column**, two in-repo remedies exist — `backfill_obs_from_source` fills masked values where a source holds them (residuals still refuse), and `drop_obs_columns` removes the column outright. A masked **`.../categories`** path is a file anndata cannot open at all: every write refuses it, and only a column drop or upstream repair clears it. Anything else is a data problem to raise with the wrangler (give the paths and per-path counts from `masked` — a sample when `masked_truncated` is true, with `masked_count` the total number of flagged *paths*, not values), never an encoding to flatten. No tool of any class leaves a partial output file.
...
The reason is worth knowing, because getting it wrong fails silently. Strip runs earlier and removes both canonical columns, so listing them here again fails the absent-name check — and the tool is all-or-nothing: that failure drops **nothing**. The non-canonical columns this scan exists for would ship, in a run that otherwise looked like it did its job. This scan covers only the names strip cannot see.

For each remaining candidate, call `get_descriptive_stats` with `attribute="obs"` and `columns` set to just the candidate names — left unset it describes every obs column, which on a file like this is 70-plus columns of noise. Build the case:
...
**Scoped to ethnicity and race, and nothing else.** Do not extend it by analogy to other fields that feel sensitive — `donor_id`, `age_*`, `disease`, geography, clinical notes. Those are legitimate HCA metadata, several are required, and proposing them for deletion is out of scope for this skill. HCA forbids self-reported ethnicity specifically; that is the whole of what this scan is for. Note the tool will not save you here: it drops any column you name, schema-named or not (#619) — a clinical field under a producer name (`dx_notes`, `donor_origin_country`) drops if you ask.
...
- **`drop_obs_columns`** — for columns the wrangler approved individually in Step 3, and only those. In this skill that means the privacy candidates from the Step 1 scan; the free-text-label case in Bucket C also points here, but it is a Bucket C decision that reaches Bucket A the same way, by the wrangler naming the column. This tool never acts on a column the skill chose by itself. Takes an **enumerated list of column names** — never a glob, never a pattern. The tool is all-or-nothing: the whole request is validated before anything is written, so if any name fails a check the file is untouched and every problem is reported together. It refuses the obs index, a name that isn't present, and a column that `uns['batch_condition']` names; schema-named columns drop like any other — whether the result is valid is `validate_schema`'s verdict (#619). Do not read that last one as general reference tracing: `batch_condition` is the only reference the tool knows about, so a column named somewhere else in `uns` can still be dropped out from under it. It deletes `uns['<column>_colors']` alongside each dropped column, since an orphaned palette is a validator error. Note it does **not** shrink the file: HDF5 doesn't reclaim freed space in place, which is one reason `compress_h5ad` runs last.
...
- **`copy_cap_annotations`** — only if the wrangler provided a CAP source file in Step 3. Copies annotation sets + `cellannotation_schema_version` + `cellannotation_metadata` from the source into the target. Partial overlap is allowed: ≤5% of cells may be missing on either side; HCA rows absent from CAP get NaN in the new CAP columns. If either side exceeds 5% missing the tool aborts — treat that as a Bucket B item and bring it back to the wrangler (usually the CAP source is stale or wrong). The gate is computed from the raw counts — `missing_from_hca.n / n_cap` and `missing_from_cap.n / n_hca`, note the crossed denominators — **not** from the `cells.missing_from_*.pct` the result reports: that `pct` is rounded to 1 dp, so a file at 5.04% reports `5.0` and still aborts. Gene-axis overlap is recorded but does not gate the copy: a CAP source with extra genes is fine.
...
- A label column is **absent** and its `*_ontology_term_id` source **resolves no label for any row** — the source is either entirely empty, or none of its values are terms the ontology recognizes. `populate_labels` refuses rather than writing an all-NaN label column, and per the item above that blocks the whole run. The same shape on the var side is a `var.index` that holds gene symbols rather than Ensembl IDs, so GENCODE resolves nothing. Both are upstream data problems, and both are independently hard `validate_schema` errors ("must not contain NaN values", "is not a valid ontology term id"), so expect the validator to name the column too — quote its error when asking upstream for the fix.
...
- **File uses the deprecated top-level CAP layout** — CAP metadata sitting in top-level `uns` keys instead of nested under `uns['cap_metadata']`. `drop_obs_columns` refuses such a file outright, whatever columns were named, and `copy_cap_annotations` refuses it as a source; the refusal fires even on a mixed-layout file that also carries a nested block. Not fixable here: the CAP file has to be re-exported with its metadata nested. Report the tool's verbatim message and bring it to the CAP curator.
...
4. Content edits, in this order: `normalize_raw`, each `replace_placeholder_values`, **labeling step** (decision tree below), `copy_cap_annotations` (if a source was supplied), and any `set_uns` approved in Step 3. Whichever labeling tool ran (if any) must precede `copy_cap_annotations` — `copy_cap_annotations` calls `validate_marker_genes`, which reads `var['feature_name']`; populating it first gives marker-gene validation canonical gene symbols to match against.
   ...
   * **Any other HCA-layout file** → run **`populate_labels`**, whether the controlled columns are partly filled or absent entirely. You do not need to classify the file first — the tool decides per column: it fills from canonical wherever the `*_ontology_term_id` source carries values, verifies a partly-filled column row by row before filling only its NaN rows, and skips an obs label column when that column *and* its `*_ontology_term_id` source are both absent. Two cases are errors rather than skips, and one error withholds the whole run (see the corresponding Bucket C items): a label column present **without** its source, and an *absent* label column whose source resolves to no label for any row (all-NaN, or term IDs the ontology doesn't recognize). It never writes an all-NaN column.
   ...
   Note it is *not* a route to a CellxGENE-ready file either: `HCALabeler` applies the HCA schema and deliberately skips the CellxGENE `uns` writes (`schema_version`, `schema_reference`, `organism`), and its preflight refuses a file that already carries `schema_version` or `schema_reference` — the only two `uns` keys it inspects. An incoming `uns['organism']` is skipped on write and is not itself a refusal; `obs['organism']` is a different matter — the labeler refuses any add-labels target column that is merely *present*, whether or not it holds values. Do not reach for it on the assumption that it serves a CellxGENE handoff.
...
For each missing marker, list it with its classification exactly as returned by the tool — `not_in_gencode` (marker symbol doesn't resolve to any GENCODE entry — typo, glob pattern, or deprecated rename), `missing_from_var` (valid symbol but not present in this file's gene set), or `known_rename` (the submitted marker **is** a valid current GENCODE symbol, but this file's `var` carries the gene under a different — usually deprecated — name, given in `var_name`, with the matching Ensembl ID in `ensembl_id`):
