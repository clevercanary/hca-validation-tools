...
- File carries `obs['self_reported_ethnicity']` or `obs['self_reported_ethnicity_ontology_term_id']` — HCA forbids these for privacy. `label_h5ad` and `validate_schema` both reject the file. Upstream needs to drop both columns and re-emit; do not strip them here.
