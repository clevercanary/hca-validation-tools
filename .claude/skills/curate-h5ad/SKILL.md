...
- **`label_h5ad`** — for files **freshly imported from CellxGENE** (edit log has an `import_cellxgene` entry from `convert_cellxgene_to_hca`). Populates `var['feature_name']` + `feature_reference` / `feature_biotype` / `feature_length` / `feature_type` from Ensembl IDs via vendored GENCODE (mirrored to `raw.var` when present), writes the seven obs ontology labels (`tissue`, `cell_type`, `assay`, `disease`, `sex`, `organism`, `development_stage`) from their `_ontology_term_id` counterparts, and writes `obs['observation_joinid']`. Unknown Ensembl IDs yield NaN across the five `feature_*` columns for that row (not an error). Preflight refuses to run if any controlled label column is pre-populated — that's the right behavior for the CellxGENE post-convert case (where add-labels already populated everything upstream and re-running would just generate a new random observation_joinid), but it blocks the HCA-tracker-imported workflow. For tracker-source files use `populate_labels` instead.
- **`populate_labels`** — **HCA-tracker-imported files** counterpart to `label_h5ad`. Per-column fill/verify for the 5 var `feature_*` columns + 7 obs ontology labels. Missing or all-NaN columns get filled from canonical (term_id → ontology label for obs; Ensembl ID → GENCODE for var); columns that already match canonical are skipped (no-op); columns that mismatch refuse the whole operation with row-level evidence. Does NOT write `observation_joinid` (HCA reserved-but-optional; non-reproducible random IDs aren't worth the audit-trail noise). Refuses on `obs['self_reported_ethnicity*']` present (run `strip_forbidden_obs_columns` first), non-human organism, CellxGENE layout (`uns['schema_version']` present), or an `import_cellxgene` edit-log entry (add-labels already ran upstream — use `label_h5ad`'s skip-narrative there instead).
...
3. Content edits, in this order: `normalize_raw`, each `replace_placeholder_values`, either `label_h5ad` (CellxGENE-imported files) or `populate_labels` (HCA-tracker-imported files — at most one of these runs per file, gated on the `import_cellxgene` edit-log entry), `copy_cap_annotations` (if a source was supplied), and any `set_uns` approved in Step 3. Whichever labeler ran must precede `copy_cap_annotations` — `copy_cap_annotations` calls `validate_marker_genes`, which reads `var['feature_name']`; populating it first gives marker-gene validation canonical gene symbols to match against.
