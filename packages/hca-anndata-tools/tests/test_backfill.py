...
def test_backfill_leaves_an_unfilled_masked_column_alone(tmp_path):
    """A masked column with nothing to fill is never rewritten, so its
    masked values must not block the run — other columns' fills proceed and
    the untouched column keeps its encoding byte-for-byte (caught by review
    on the first cut, which refused the whole run)."""
    target = _make_h5ad(
        tmp_path / "target.h5ad",
        TARGET_IDS,
        {"library_id": TARGET_LIB, "run_id": ["R"] * len(TARGET_IDS)},
    )
    source = _make_h5ad(
        tmp_path / "source.h5ad",
        SOURCE_IDS,
        {"library_id": SOURCE_LIB, "run_id": ["R"] * len(SOURCE_IDS)},
    )
    with h5py.File(target, "r+") as f:
        make_plain_string_column(f["obs"], "run_id", ["R"] * len(TARGET_IDS))
        make_nullable_string_array(f["obs"], "run_id")
        # Mask c5 — a cell with no source row, so the mask cannot be filled
        # and the column has zero fills overall.
        f["obs/run_id/mask"][4] = True
