...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "refuses a non-categorical column, a CAP annotation-set column, a column "
        "and a derived label whose '<col>_ontology_term_id' is present (correct the "
        "term IDs and regenerate with populate_labels instead); it trims the merged-away "
        "category's entry from uns['<col>_colors'] so the surviving colours stay aligned, "
        "and reports regenerate_labels_required when you merged a term-ID column, "
