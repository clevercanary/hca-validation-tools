...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "carrying a known CAP suffix (a producer column merely containing '--' is reported, not "
        "deleted), and removed columns' _colors palettes; the remediation for files the toolkit "
        "refuses as deprecated-top-level-CAP-layout, and the precursor to a fresh "
        "copy_cap_annotations run from a current CAP export. It only undoes our own imports: CAP "
        "uns metadata is stripped only when the edit log carries import_cap_annotations, so raw "
        "CAP exports are refused (as are CellxGENE-layout files), and it errors without writing "
        "when the file has no CAP material, "
