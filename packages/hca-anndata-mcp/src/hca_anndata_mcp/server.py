...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "carrying a per-category uns['<col>_colors'] palette (positionally aligned, so "
        "dropping a category would shift every colour after it — drop the palette "
        "first), and a derived label whose '<col>_ontology_term_id' is present (correct "
        "the term IDs and regenerate with populate_labels instead), "
